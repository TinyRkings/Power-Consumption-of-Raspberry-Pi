...
//generate initial centroids concurrently
void initcentroidsParallel(struct mycentroid* centroids){
	for (int i = 0; i<number_centers;i++){
		for (int j=0; j< Dimensions; j++){
				//input >> centroids[i].values[j];
				centroids[i].values[j]= rand()%1067;
		}
	...
	for (int i=0; i< number_centers; i++) {
		for (int j=0; j< Dimensions; j++)
			if(centroids[i].cluster!=0){
				centroids[i].values[j] = centroids[i].values[j] / centroids[i].cluster;
			}
	}
}
